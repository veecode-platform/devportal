...
export class PostgresPartnerRepository implements IPartnerRepository {
  ...
  async getPartnerByUser(email:string): Promise<Partner[] | void> {
    const partner = await this.db<Partner>('partner').where("email", email).select('*').catch(error => console.error(error));
    return partner;
  }

  async getPartner(): Promise<Partner[]> {
    const partner = await this.db<Partner>('partner').select('*').catch(error => console.error(error));
...
// method get one partner by id
  async getPartnerById(id: string): Promise<Partner | string> {
    const partner = await this.db<Partner>('partner').where('id', id).limit(1).select().catch(error => console.error(error));
...
// method to delete partner
  async deletePartner(id: string): Promise<void> {
   await this.db<Partner>('partner').where('id', id).del().catch(error => console.error(error));
  }
  ...
  async createPartner(partnerDto: PartnerDto): Promise<Partner | string> {
    ...
    const createdPartner = await this.db('partner').insert(data).catch(error => console.error(error));
    ...
    async updatePartner(id: string, partnerDto: PartnerDto): Promise<Partner | string> {
      ...
      const data =await PartnerMapper.toPersistence(partner);
      const updatedPartner = await this.db('partner').where('id', id).update(data).catch(error => console.error(error));
 ...
 // async function to patch partial  partner object partial class type
  async patchPartner(id: string, partnerDto: PartnerDto): Promise<Partner | string> {
    ...
    const patchedPartner = await this.db('partner').where('id', id).update(partnerDto).catch(error => console.error(error));
