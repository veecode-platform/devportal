  ...
  const authMiddleware: RequestHandler = async (
    ...
    try {   
      ...
      try {
        ...
        if(skipHeader) await appEnv.tokenManager.authenticate(skipHeader);
        else await appEnv.tokenManager.authenticate(token);
      }
      if (!req.headers.authorization) {
        // Authorization header may be forwarded by plugin requests
        req.headers.Authorization = `Bearer ${token}`;
      }
      if (token && token !== req.cookies?.token && !skipHeader) {
        setTokenCookie(res, {
          token,
          secure,
          cookieDomain,
